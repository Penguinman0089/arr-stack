...
A Docker Compose media automation stack **that runs on a NAS**, not on this local machine. Users request TV shows/movies via Seerr → Sonarr/Radarr search for them → qBittorrent/SABnzbd download them (through VPN) → media appears in Plex ready to watch.
...
- **Remote** (via Cloudflare Tunnel): Plex, Seerr, WireGuard, Traefik dashboard
